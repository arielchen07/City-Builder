...
Road:
The road button is a work in progress (WIP), and users can press the 'r' key for placement. More updates expected in the future.
Housing:
Clicking on the housing button directs users to an inventory containing all types of houses. Currently, only the Single Family House is available. More updates expected in the future.
Utilities:
Clicking on the utilities button leads to a submenu with subcategories such as energy, sewage, and water. Each subcategory offers different types of buildings to choose from.
...
Server:
	...
	For macOS:
Installing .NET:  open terminal and run 
-brew install --cask dotnet-sdk
Installing Node.js: run: brew install node
For Windows:
Installing .NET: 
